...
void Fill_Field(QPainter* p, QPixmap* map, QImage* image, int cellPosX, int cellPosY, int onePixDistance, int TxPower, int antGain, float freq) {
    float fi3db = 65.0; // Угол направления
    float Amax = 30.0; // Максимальный уровень мощности

        for(int i = 0; i < maxX; i++) {
            for(int j = 0; j < maxY; j++) {
                float distance = sqrt(pow(abs(cellPosX - i), 2) + pow(abs(cellPosY - j), 2));
                distance *= onePixDistance;
                float sigPower = TxPower + antGain - PL(freq, distance);
                float angle = atan2(j - cellPosY, i - cellPosX) * (180.0 / M_PI); // Вычисляем угол направления

                float antennaAngle = abs(angle - 90); // Угол направления антенны относительно вертикали

                if (antennaAngle <= 180) {
                    float fiRatio = antennaAngle / fi3db;
                    float term = 12.0 * pow(fiRatio, 2);
                    float Adb= -std::min(term, Amax);
